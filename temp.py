...
Curseur.execute("""INSERT INTO new  (tagIG,AVG(TO_NUMBER(discordID),pseudoIG,thIG,donne,recu,nbattaqueshdv,perfhdv,bihdv,onehdv,blackhdv,"nbattaqueshdv-1","perfhdv-1","bihdv-1","onehdv-1","blackhdv-1") SELECT scores.tag,MAX(scores.th),MIN(pseudoIG),IdDiscord,SUM(donne),SUM(recu),AVG(perf+bi+one+black),AVG(perf),AVG(bi),AVG(one),AVG(black),AVG(perfdips+bidips+onedips+blackdips),AVG(perfdips),AVG(bidips),AVG(onedips),AVG(blackdips) FROM scores LEFT JOIN nommage ON  scores.tag=nommage.tagIG AND scores.th=nommage.th GROUP BY scores.tag,IDdiscord""")
connectionBDD.close()
