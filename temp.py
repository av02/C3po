...
Curseur.execute("""INSERT INTO new  (tagIG,thIG,pseudoIG,discordID,donne,recu,nbattaqueshdv,perfhdv,bihdv,onehdv,blackhdv,"nbattaqueshdv-1","perfhdv-1","bihdv-1","onehdv-1","blackhdv-1") SELECT scores.tag,MAX(scores.th),MIN(pseudoIG),AVG(TO_NUMBER(IdDiscord)),SUM(donne),SUM(recu),AVG(perf+bi+one+black),AVG(perf),AVG(bi),AVG(one),AVG(black),AVG(perfdips+bidips+onedips+blackdips),AVG(perfdips),AVG(bidips),AVG(onedips),AVG(blackdips) FROM scores LEFT JOIN nommage ON  scores.tag=nommage.tagIG AND scores.th=nommage.th GROUP BY scores.tag""")
connectionBDD.close()
