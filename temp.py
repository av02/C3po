...
Curseur.execute("""INSERT INTO new  (tagIG,thIG,pseudoIG,discordID,donne,recu,nbattaqueshdv,perfhdv,bihdv,onehdv,blackhdv,"nbattaqueshdv-1","perfhdv-1","bihdv-1","onehdv-1","blackhdv-1") SELECT scores.tag,MAX(scores.th),MIN(pseudoIG),MIN(IdDiscord),SUM(donne),SUM(recu),perf+bi+one+black,perf,bi,one,black,perfdips+bidips+onedips+blackdips,perfdips,bidips,onedips,blackdips FROM scores LEFT JOIN nommage ON  scores.tag=nommage.tagIG AND scores.th=nommage.th GROUP BY scores.tag""")
connectionBDD.close()
