...
async def gc(DiscordClient, message,args):#TODO: ajouter le cas de l'autotag, pas besoin de s'autotag
    ...
    liste_profils.sort(key=lambda p:p.town_hall*1000+p.exp_level)   
    for player in liste_profils:    
        rep.add_field(name=player.name,value="<:HdvBot:884202091793506324> Hdv : {} \n<:ExpBot:884202964896608266> Niveau : {} \n<:TagBot:884204003070705754> Tag : {}\n🛡️ Clan:{}".format(player.town_hall,player.exp_level,player.tag,player.clan.name if player.clan is not None else "Hors Empire"))#,ligne_BDD[7]/(ligne_BDD[6]+0.000000000001)*100))
