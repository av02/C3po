...
async def gc(DiscordClient, message,args):#TODO: ajouter le cas de l'autotag, pas besoin de s'autotag
    ...
    async for player,ligne_BDD in zip(DiscordClient.cocClient.get_players(tags),liste_comptes):
        rep.add_field(name=player.name,value="<:HdvBot:884202091793506324> Hdv : {} \n<:ExpBot:884202964896608266> Niveau : {} \n<:TagBot:884204003070705754> Tag : {}\n🛡️ Clan:{}\n<:3Stars:898336084897984574> %perf:{} %".format(player.town_hall,player.exp_level,player.tag,player.clan.name if player.clan is not None else "dehors",ligne_BDD[7]/(ligne_BDD[6]+0.000000000001)*100))
