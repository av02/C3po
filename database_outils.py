...
class appelsBDD:
    ...
    def classement_dons(self, limit, *, clan=None) -> list:
        ...
        return self.appel_bdd("""SELECT  discordid ,MIN(tagig) AS TAGIG,MAX(pseudoig) AS pseudo,SUM(donne) AS DON ,SUM(recu) AS recu,(SUM(donne)+0.00000000000001)/(recu+0.00000000000001) AS ratio FROM new WHERE  discordid IS NOT NULL {} GROUP BY discordid ORDER BY DON DESC LIMIT {}""".format(requete_clan, limit))
