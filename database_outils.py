...
class appelsBDD:
    ...
    def appel_bdd(self,instruction:str,commit=True)->list:
        ...
        """
        connectionBDD= psycopg2.connect(self.bddlink)#(dbname=self.bddlink,sslmode='require')
    ...
    def check_presence_database(self,tag,th,pseudo,clan):
        ...
        """
        if len(self.appel_bdd("SELECT * FROM nommage WHERE tagIG = '{}'".format(tag)))==0:
            self.appel_bdd("INSERT INTO nommage (tagIG,pseudoIG,thIG,clan) VALUES ('{}','{}',{},'{}')".format(tag,pseudo,th,clan))
        return

    @attrapeur_d_exception
    def add_discord_id(self,tag,id):
        """ajoute l'identifiant discord associé a un compte
        ecrase le tag précédent
        """
        self.appel_bdd("UPDATE nommage SET discordID={} WHERE tagIG='{}'".format(id,tag))
        return 
    ...
    def get_discord_id(self,tag)->int:#TODO controler si non enregistré
        ...
        """
        return int(self.appel_bdd("SELECT discordID FROM nommage WHERE tagIG='{}'".format(tag))[0][0])

    @attrapeur_d_exception
    def edit_pseudo(self,tag,pseudo):
        """modifie le pseudo du joueur, ecrase le précdent
        """
        self.appel_bdd("UPDATE nommage SET pseudoIG='{}' WHERE tagIG='{}'".format(pseudo,tag))

    @attrapeur_d_exception
    def edit_clan(self,tag,clan=None):  
        """met a jour le clan du joueur avec son nouveau clan"""
        self.appel_bdd("""UPDATE nommage SET clan='{}' WHERE tagIG='{}'""".format(clan,tag))
    ...
    def up_hdv(self,tag,th):
        ...
        ecrase le précdent
        """
        stats=self.appel_bdd("SELECT thIG,nbattaqueshdv,perfhdv,bihdv,onehdv,blackhdv FROM nommage WHERE tagIG='{}'".format(tag))[0]
        print("#1")
        if stats[0]!=th:
            self.appel_bdd("UPDATE nommage SET thIG={}, 'nbattaqueshdv-1'={},'perfhdv-1'={},'bihdv-1'={},'onehdv-1'={},'blackhdv-1'={}, nbattaqueshdv=0,perfhdv=0,bihdv=0,onehdv=0,blackhdv=0,perfdefhdv=0,nbdefhdv=0 WHERE tagIG='{}'".format(th,stats[1],stats[2],stats[3],stats[4],stats[5],tag))
    
    @attrapeur_d_exception
    def add_score_gdc(self,tag,etoiles,dips=False):
        """
        ...
        valeurs_anterieures = self.appel_bdd("""SELECT nbattaqueshdv{},{} 
                                            FROM nommage WHERE tagIG='{}'""".format(fin_dips,
                                                                                _transcription_etoiles_strbdd(etoiles)+fin_dips,
                                                                                tag))[0]
        print("iteration n°:3")
        self.appel_bdd("""UPDATE nommage SET 'nbattaqueshdv{}'={}, '{}'={} 
    ...
    @attrapeur_d_exception
    def add_def_gdc(self,tag,perf:bool):#TODO ajouter clause d'egalité d'hdv????
        ...
        """
        valeurs_anterieures=self.appel_bdd("""SELECT nbdefhdv,perfdefhdv FROM nommage WHERE tagIG='{}'""".format(tag))[0]
        self.appel_bdd("""UPDATE nommage SET nbdefhdv={},perfdefhdv={} WHERE tagIG='{}'""".format(valeurs_anterieures[0]+1,valeurs_anterieures[1]+int(perf),tag))
        
    @attrapeur_d_exception
    def add_don(self,tag,dons):
        """ajoute dons au dons associés au tag
        """
        valeur_anterieur=self.appel_bdd("""SELECT donne FROM nommage 
                                        WHERE tagIG='{}'""".format(tag))[0][0]
        self.appel_bdd("""UPDATE nommage SET donne={} WHERE tagIG='{}'""".format(int(valeur_anterieur)+dons,tag))
    
    @attrapeur_d_exception
    def add_recu(self,tag,recu):
        """ajoute recu au recu associés au tag
        """
        valeur_anterieur=self.appel_bdd("""SELECT recu FROM nommage 
                                        WHERE tagIG='{}'""".format(tag))[0][0]
        self.appel_bdd("""UPDATE nommage SET recu={} WHERE tagIG='{}'""".format(int(valeur_anterieur)+recu,tag))
    ...
    def classement_dons(self,limit,*,clan=None)->list:
        ...
        return self.appel_bdd("""SELECT discordID,tagIG,pseudoIG,donne,recu,donne/recu FROM nommage {} ORDER BY donne/recu DESC LIMIT {}""".format(requete_clan,limit))
    ...
    def get_comptes_coc(self,idDiscord):
        ...
        (tagIG [0],discordID [1],pseudoIG [2],thIG [3], donne[4],recu [5],nbattaques[6],perf[7],bi[8],one[9],
		black[10],nbdips[11],perfdips[12],bidips[13],onedips[14],blackdips[15],perfdefs[16],nbdefs[17],clan[18)
        """
        return self.appel_bdd("""SELECT * FROM nommage WHERE discordID={}""".format(idDiscord))
    ...
    def get_data(self,tag)->tuple:
        ...
        """
        return self.appel_bdd("""SELECT * FROM nommage WHERE tagIG='{}'""".format(tag))[0]
    ...
    def get_classement_attaques(self,hdv,*,dips=False,limit=10,clan=None,nb_etoiles=3)->list:
        ...
        return self.appel_bdd("""SELECT tagIG,discordID,pseudoIG,nbattaques{1},perfhdv{1},bihdv{1},onehdv{1},blackhdv{1} FROM nommage WHERE thIG={0}{3} ORDER BY 
    ...
    def get_classement_defenses(self,hdv,*,limit=10,clan=None)->list:
        ...
        requete_clan= ",clan="+clan if clan is not None else ""
        return self.appel_bdd("""SELECT tagIG,discordID,pseudoIG,nbdefhdv,perfdefhdv,clan FROM nommage
    ...
    def reduire_dons(self,facteur=2):
        ...
        self.appel_bdd("""UPDATE nommage SET donne=donne/{0},recu=recu/{0}""".format(facteur))
